...
use context::Context;
use display_list::{Au, ClippingRegion, Color, DisplayItem, TRANSPARENT_GREEN, WHITE};
...
impl Batch {
    ...
    fn add_dummy_buffer_gamma(&mut self, count: usize) {
        self.buffer_gamma.extend(iter::repeat(Point2D::new(0.0, 0.0)).take(count))
    }
...
impl Batcher {
    ...
    pub fn add(&mut self, context: &Context, display_item: DisplayItem) {
